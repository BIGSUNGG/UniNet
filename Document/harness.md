...
1. 코드나 개발 환경이 수정되었을 때(기능 추가/수정/제거, 리팩토링, 버그 수정, 빌드·설정 변경 등) → 검토를 위해 reviewer 서브에이전트를 호출하는 이유를 사용자에게 알리고 호출 여부를 질문한 뒤, 허락한 경우에만 diff·맥락을 전달해 구조·보안·속도 포함 검토 요청
